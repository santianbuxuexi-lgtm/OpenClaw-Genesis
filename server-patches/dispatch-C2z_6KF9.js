...
import { runGenesisDedicatedAccountReply, GENESIS_MUTEX_ENV_KEY } from "./genesis-qq-router.mjs";
...
async function dispatchReplyFromConfig(params) {
	...
	*/
	...
	if (process.env[GENESIS_MUTEX_ENV_KEY] !== "1") {
		const dedicatedGenesisReply = runGenesisDedicatedAccountReply({
			ctx,
			cfg
		});
		if (dedicatedGenesisReply) {
			let queuedFinal = false;
			let routedFinalCount = 0;
			if (shouldRouteToOriginating && originatingChannel && originatingTo) {
				const result = await routeReplyRuntime.routeReply({
					payload: dedicatedGenesisReply,
					channel: originatingChannel,
					to: originatingTo,
					sessionKey: ctx.SessionKey,
					accountId: ctx.AccountId,
				...
				if (!result.ok) logVerbose(`dispatch-from-config: route-reply (genesis dedicated account) failed: ${result.error ?? "unknown error"}`);
			} else queuedFinal = dispatcher.sendFinalReply(dedicatedGenesisReply);
			const counts = dispatcher.getQueuedCounts();
			counts.final += routedFinalCount;
			recordProcessed("completed", { reason: "genesis_dedicated_account" });
			markIdle("genesis_dedicated_account");
			return {
				queuedFinal,
				counts
			};
